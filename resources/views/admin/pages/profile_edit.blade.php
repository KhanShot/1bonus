@extends('layouts.main', ['page' => __('User Profile'), 'pageSlug' => 'profile'])
                ...
                <form method="post" action="{{ route('profile.update') }}" autocomplete="off">
                    <div class="card-body">
                            ...
                            <div class="form-group{{ $errors->has('email') ? ' has-danger' : '' }}">
                                ...
                                <input type="email" name="email" class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" placeholder="{{ __('Почта') }}" value="{{ old('email', auth()->user()->email) }}">
                ...
                <form method="post" action="{{ route('profile.password') }}" autocomplete="off">
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            <label>{{ __('Подтвердить новый пароль') }}</label>
                            <input type="password" name="password_confirmation" class="form-control" placeholder="{{ __('Подтвердить новый пароль') }}" value="" required>
                        </div>
                    </div>
                    <div class="card-footer">
                        <button type="submit" class="btn btn-fill btn-primary">{{ __('Изменить') }}</button>
                    </div>
                </form>
            </div>
        </div>
